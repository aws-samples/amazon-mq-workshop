...
class DeadLetterQueue:
    def __init__(self, *, max_receive_count: jsii.Number, queue: "IQueue") -> None:
        ...
        :param max_receive_count: The number of times a message can be unsuccessfully dequeued before being moved to the dead-letter queue.
...
class RedriveAllowPolicy:
    def __init__(
        ...
        *,
        ...
        :param redrive_permission: Permission settings for source queues that can designate this queue as their dead-letter queue. Default: - ``RedrivePermission.BY_QUEUE`` if ``sourceQueues`` is specified,``RedrivePermission.ALLOW_ALL`` otherwise.
        :param source_queues: Source queues that can designate this queue as their dead-letter queue. When ``redrivePermission`` is set to ``RedrivePermission.BY_QUEUE``, this parameter is required. You can specify up to 10 source queues. To allow more than 10 source queues to specify dead-letter queues, set the ``redrivePermission`` to ``RedrivePermission.ALLOW_ALL``. When ``redrivePermission`` is either ``RedrivePermission.ALLOW_ALL`` or ``RedrivePermission.DENY_ALL``, this parameter cannot be set. Default: - Required when ``redrivePermission`` is ``RedrivePermission.BY_QUEUE``, cannot be defined otherwise.
        ...
        self._values: typing.Dict[builtins.str, typing.Any] = {}
        if redrive_permission is not None:
            self._values["redrive_permission"] = redrive_permission
        if source_queues is not None:
            self._values["source_queues"] = source_queues

    @builtins.property
    def redrive_permission(self) -> typing.Optional["RedrivePermission"]:
        ...
        '''
        result = self._values.get("redrive_permission")
    ...
    def __repr__(self) -> str:
        return "RedriveAllowPolicy(%s)" % ", ".join(
            k + "=" + repr(v) for k, v in self._values.items()
        )


@jsii.enum(jsii_type="aws-cdk-lib.aws_sqs.RedrivePermission")
class RedrivePermission(enum.Enum):
